...
public class PitMojoTest extends BasePitMojoTest {
  ...
  public void testConfigureEnvironmentVariable() throws Exception {
    ...
    var mojo = createPITMojo(createPomWithConfiguration("\n"
  ...
  private void setupCoverage(long mutationScore, int lines, int linesCovered)
    ...
    final var stats = new MutationStatistics(scores, 100, mutationScore, 100, 0, Collections.emptySet());
    var sum = new CoverageSummary(lines, linesCovered, 0);
    final var cs = new CombinedStatistics(stats, sum, Collections.emptyList());
    when(
  ...
  private void setupTestStrength(long totalMutations, long mutationDetected, long mutationsWithCoverage)
          throws MojoExecutionException {
    Iterable<Score> scores = Collections.<Score>emptyList();
    final var stats = new MutationStatistics(scores, totalMutations, mutationDetected, mutationsWithCoverage, 0, Collections.emptySet());
    var sum = new CoverageSummary(0, 0, 0);
    final var cs = new CombinedStatistics(stats, sum, Collections.emptyList());
    when(
            this.executionStrategy.execute(any(File.class),
                    any(ReportOptions.class), any(PluginServices.class), anyMap()))
  ...
  private void setupSuvivingMutants(long survivors)
    ...
    Iterable<Score> scores = Collections.<Score>emptyList();
    var detected = 100;
    final var stats = new MutationStatistics(scores, detected + survivors, detected, detected + survivors, 0, Collections.emptySet());
    var sum = new CoverageSummary(0, 0, 0);
    final var cs = new CombinedStatistics(stats, sum, Collections.emptyList());
    when(
        this.executionStrategy.execute(any(File.class),
